...
export default function HouseBreakdownModal({ houseId, onClose }) {
  ...
  useEffect(() => {
    if (!houseId) return;
    setLoading(true);
    fetch(`/api/houses/${houseId}/breakdown`, { cache: 'no-store' })
      .then(res => res.json())
      .then(resData => {
        setData(resData);
        setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div 
          ...
          style={{ backgroundColor: house.color_hex || '#10B981' }}
        >
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-slate-50/50">
          ...
          {loading ? (
            <div className="p-12 text-center text-slate-500 font-bold text-sm">
              Loading House point calculations & results...
            </div>
          ) : (
            <>
              {/* Summary Stats Grid */}
              <div className="grid grid-cols-4 gap-3 text-center">
                ...
                <div className="p-3 rounded-2xl bg-emerald-600 text-white shadow-md">
                  ...
                  <span className="text-2xl font-black font-mono block">{house.total_points || 0}</span>
