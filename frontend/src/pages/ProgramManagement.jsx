...
export default function ProgramManagement() {
  ...
  const filteredPrograms = programs.filter(p => {
    ...
    if (selectedStage !== 'All') {
      const isOff = (p.stage_type || p.code || '').toLowerCase().includes('off');
      if (selectedStage === 'On Stage' && isOff) return false;
      if (selectedStage === 'Off Stage' && !isOff) return false;
    }
    ...
    if (searchQuery.trim()) {
      ...
      if (!pName.includes(q) && !pCode.includes(q) && !pCat.includes(q)) return false;
    }
  ...
  return (
    <div className="space-y-6" onClick={() => setActiveDropdown(null)}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {isAdmin && (
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-md transition btn-interactive shrink-0"
          >
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-200 shadow-xs">
        ...
        {/* Search & Stage Type Quick Filters */}
        <div className="flex items-center space-x-2 shrink-0">
          <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-[11px] font-bold">
            {['All', 'On Stage', 'Off Stage'].map(stg => (
              <button
                key={stg}
                type="button"
      ...
      {filteredPrograms.length === 0 ? (
        <div className="glass-panel p-12 text-center rounded-3xl bg-white border border-slate-200 shadow-xs">
          ...
          <h3 className="text-base font-extrabold text-slate-800 mb-1">
            No Programs Found in <span className="text-emerald-600 font-black">"{selectedCategory}"</span>
          </h3>
          <p className="text-xs text-slate-500 font-medium max-w-sm mx-auto mb-4">
            There are no competition items matching this category filter or search query.
          </p>
          <button
            onClick={() => { setSelectedCategory('All'); setSelectedStage('All'); setSearchQuery(''); }}
          ...
          >
            Show All Programs ({programs.length})
          </button>
        </div>
      ) : (
