...
function translateQuery(sql, params = []) {
  ...
  if (pgSql.includes('INSERT OR REPLACE INTO')) {
    if (pgSql.includes('INSERT OR REPLACE INTO settings')) {
      ...
      pgSql = pgSql.replace('INSERT OR REPLACE INTO students', 'INSERT INTO students') + ' ON CONFLICT (id) DO UPDATE SET student_id = EXCLUDED.student_id, admission_no = EXCLUDED.admission_no, name = EXCLUDED.name, category_name = EXCLUDED.category_name, arabic_name = EXCLUDED.arabic_name, photo = EXCLUDED.photo, gender = EXCLUDED.gender, dob = EXCLUDED.dob, age = EXCLUDED.age, class_name = EXCLUDED.class_name, division = EXCLUDED.division, house_id = EXCLUDED.house_id, parent_name = EXCLUDED.parent_name, phone = EXCLUDED.phone, email = EXCLUDED.email, address = EXCLUDED.address, qr_code = EXCLUDED.qr_code, is_archived = EXCLUDED.is_archived';
    } else if (pgSql.includes('INSERT OR REPLACE INTO programs')) {
      pgSql = pgSql.replace('INSERT OR REPLACE INTO programs', 'INSERT INTO programs') + ' ON CONFLICT (id) DO UPDATE SET code = EXCLUDED.code, name = EXCLUDED.name, category_id = EXCLUDED.category_id, age_group = EXCLUDED.age_group, type = EXCLUDED.type, gender_category = EXCLUDED.gender_category, stage_type = EXCLUDED.stage_type, venue_id = EXCLUDED.venue_id, program_date = EXCLUDED.program_date, start_time = EXCLUDED.start_time, end_time = EXCLUDED.end_time, max_participants = EXCLUDED.max_participants, status = EXCLUDED.status, duration_minutes = EXCLUDED.duration_minutes, is_archived = EXCLUDED.is_archived';
...
async function initDb() {
  if (isPg) {
    console.log('[DB] PostgreSQL (Neon) detected. Initializing tables, indexes and sequences...');
    ...
    // Ensure default base tables (users, houses, categories, venues) ONLY if they are completely empty (0 rows)
    ...
    const houseCount = await get('SELECT COUNT(*) as count FROM houses');
    if (!houseCount || parseInt(houseCount.count, 10) === 0) {
    ...
    if (!categoryCount || parseInt(categoryCount.count, 10) === 0) {
      ...
      for (const cat of requiredCategories) {
        await run('INSERT INTO categories (id, name, min_age, max_age, description) VALUES (?, ?, ?, ?, ?) ON CONFLICT DO NOTHING',
    ...
    console.log('[DB] Neon PostgreSQL initialized. Single source of truth active (NO snapshots restored).');
    return;
  }

  // --- SQLite Local Development Only ---
  console.log('[DB] Initializing SQLite database schema for local development...');
  ...
  // Ensure default seeds exist in SQLite if empty
  ...
  console.log(`[DB] Database initialized successfully. Mode: SQLite (Local Dev)`);
}
